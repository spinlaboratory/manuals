...
### Neat BDPA Crystals
Neat BDPA can be used in EPR spectroscopy. For example, individual crystals, picked from the bottle make a good standard to observe an FID or if only a small spec of an EPR active substance is required for example to map out the B<sub>1</sub> field of a resonator. However, be careful when handling neat BDPA. The smallest contaminations can result in large EPR signals, which will require extensive cleaning.
