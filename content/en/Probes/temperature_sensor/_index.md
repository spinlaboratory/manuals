...
#### Temperature Sensor
The Bruker QLP probe is equipped with an an integrated [Cernox Temperature Sensor](https://www.lakeshore.com/products/categories/temperature-products/cryogenic-temperature-sensors) to measure the temperature close to the sample/resonator. This is especially helpful when using a liquid cryogen-free (dry) cryostat that uses an exchange gas. The temperature senor is located close to the actual sample position for an accurate reading.

Typically, the QLP probe will be shipped with a cable to connect the probe to a temperature controller. Please make sure you inform Bruker about the temperature controller you intend to use. For reference the pinout for two of the most commonly used temperature controllers is given in the next section.
...
2. Rename the CTC100 calibration file (this is a .txt file). The name of the file should be the name of the channel, with any spaces removed, plus .txt extension. If you don't have a .txt file with the sensor calibration data contact us at epr-applications@bruker.com.
